...
# SO(n) transform
# transform = HealpixSOPyramid(NSIDE, method='integer', nside_at_levels=[8, 4, 2])
# transform.to(torch_training_device)
# transform, history, validation = learn_transformation(
#     transform, 
#     data=torch.from_numpy(np.asarray(y)), 
#     epochs=100,
#     device=torch_training_device,
#     # patience=300
# )
# transform.to('cpu')
...
nle = MAFSurjectiveNeuralLikelihood(
    ndim, 
    n_layers=8, 
    data_reduction_factor=0.8, # lower implies larger reduction
    decoder_distribution='students_t'
)
