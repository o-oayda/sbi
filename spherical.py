...
class sphericalConv(nn.Module):
    ...
    def forward(self, x):
        # add extra zero at end of last axis
        x2 = F.pad(x, (0,1,0,0,0,0), mode='constant', value=0.0)
        # for masked tensors, True denotes a valid (unmasked) pixel
        # len1, len2, _ = x.shape
        # padding = torch.masked.as_masked_tensor(
            # torch.zeros((len1, len2, 1)),
            # mask=torch.ones((len1, len2, 1)).bool()
        # )
        # x2 = torch.cat([x, padding], dim=-1)
